  ...
  const handleCalculate = async () => {
    ...
    try {
      if (window.openai?.callTool) {
        const response = await window.openai.callTool('subtract', { a: numA, b: numB });
        ...
        setResult(diff);
        window.openai?.setWidgetState?.({ lastResult: diff });
      }
    } catch (err) {
      console.error(err);
      setResult(numA - numB);
...
const container = document.getElementById('subtract-root');
if (container) {
  const root = createRoot(container);
  root.render(<SubtractCalculator />);
}
